...
extends Merge {
     ...
     *     <li>The code then checks if the element at index {@code mid} is less than or equal to the element at index {@code start}.
     *     If this condition is true, it means that the two sublists are already sorted, and there is no need to perform any merging.
     *     In that case, the method simply returns.</li>
     *     <li>If the condition in step 3 is false, indicating that merging is necessary, a {@code while} loop is used
     ...
     *     If the element at {@code left} is less than or equal to the element at {@code start},
     *     it means that the element at {@code left} is already in its correct sorted position,
     *     and we can simply move to the next element in the {@code left} sublist by incrementing the {@code left} variable.</li>
     *     <li>If the element at {@code left} is greater than the element at {@code start},
     *     it means that the element at {@code start} needs to be inserted into its correct position in the {@code left} sublist.
     *     The code creates a temporary variable value to store the value at index {@code start},
     *     and an index variable {@code index} to keep track of the position where value will be inserted.</li>
     *     <li>A {@code while} loop is used to shift the elements from {@code index} to {@code left} in the {@code list} one position to the {@code right},
     *     effectively making space for {@code value} to be inserted at the correct position.</li>
     *     <li>After shifting the elements, {@code value} is inserted at index {@code left}, and both {@code left}
     *     and {@code start} are incremented to move to the next elements in their respective sublists.</li>
     *     <li>The {@code while} loop continues until either the {@code left} sublist or the {@code start} sublist is exhausted, i.e.,
    ...
    /**
     * {@code mergingDec} is a Java implementation of the merging step in the merge sort algorithm.
     * It takes an list of {@link java.lang.Comparable Comparable} objects, along with the indices representing
     * the left, middle, and right boundaries of the sublist being merged.
     * <ul>
     *     <li>The {@link java.lang.SuppressWarnings @SuppressWarnings}{@code ("unchecked")} annotation is used to suppress compiler warnings related to type
     *     safety when using the {@link java.lang.Comparable#compareTo(Object) compareTo} method.</li>
     ...
     *     If the element at {@code left} is more than or equal to the element at {@code start},
     *     it means that the element at {@code left} is already in its correct sorted position,
     *     and we can simply move to the next element in the {@code left} sublist by decrementing the {@code left} variable.</li>
     *     <li>If the element at {@code left} is smaller than the element at {@code start},
     *     it means that the element at {@code start} needs to be inserted into its correct position in the {@code left} sublist.
     *     The code creates a temporary variable value to store the value at index {@code start},
     *     and an index variable {@code index} to keep track of the position where value will be inserted.</li>
     ...
     *     Inside the {@code while} loop, it compares the elements at {@code list.set(start)} and {@code list.set(left)}.
     ...
     *     <li>If the element at {@code list.set(start)} is smaller than the element at {@code list.set(left)}, it means that
     ...
     *     <li>After each element is processed, it increments {@code left} to move to the next position in the first sublist,
     *     increments {@code mid} to account for the element insertion,
     *     and increments {@code start} to move to the next position in the second sublist.</li>
     *     <li>Once the while loop exits, the merging is complete, and the two sublists are merged into a single sorted sublist.</li>
     * </ul>
     ...
     */
    @Override
    public <L extends Comparable> void merging(List<L> list, int left, int mid, int right, SortFunctional<Comparable> functional) {
        int start = (mid + 1);
        SortFunctional<Comparable> functionalAddEquals = functionalComparableToAddEquals(functional);
