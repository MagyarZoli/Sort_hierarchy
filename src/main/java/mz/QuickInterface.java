...
extends Sorter<T>, SortSwap<T> {
     ...
     * @see         mz.QuickInterface#quickInc(List, int, int)
     */
    default <L extends T> void quickInc(List<L> list) {
        quickInc(list, 0, (list.size() - 1));
    }
     ...
     * The base case of the recursion is when the subarray has only one element, in which case it is considered already sorted.
     * @param       list to be arranged.
     * @see         mz.QuickInterface#quickDec(List, int, int)
     */
    default <L extends T> void quickDec(List<L> list) {
        quickDec(list, 0, (list.size() - 1));
    }
     ...
     * @param       list to be arranged.
     * @param       functional lambda expression for comparison.
     * @see         mz.QuickInterface#quick(List, int, int, SortFunctional)
     */
    default <L extends T> void quick(List<L> list, SortFunctional<T> functional) {
        quick(list, 0, (list.size() - 1), functional);
    }
     ...
     *     with left as the start index and {@code (partitionIndex - 1)} as the end index.</li>
     *     <li>The second recursive call sorts the right partition of the list by calling {@code quickInc} with
     *     {@code (partitionIndex + 1)} as the start index and {@code right} as the end index.</li>
     *     <li>This process is repeated until the entire list is sorted.</li>
     * </ul>
     * @param       list to be arranged.
     ...
     * @see         mz.QuickInterface#partitionInc(List, int, int)
     */
    default <L extends T> void quickInc(List<L> list, int left, int right) {
        if (left < right) {
            int partitionIndex = partitionInc(list, left, right);
            quickInc(list, left, (partitionIndex - 1));
            quickInc(list, (partitionIndex +1), right);
        }
    }
     ...
     *     with left as the start index and {@code (partitionIndex - 1)} as the end index.</li>
     *     <li>The second recursive call sorts the right partition of the list by calling {@code quickDec} with
     *     {@code (partitionIndex + 1)} as the start index and {@code right} as the end index.</li>
     *     <li>This process is repeated until the entire list is sorted.</li>
     * </ul>
     * @param       list to be arranged.
     * @param       left the value in the list must be smaller than a {@code right} parameter.
     * @param       right the value in the list must be greater than a {@code left} parameter.
     * @see         mz.QuickInterface#partitionDec(List, int, int)
     */
    default <L extends T> void quickDec(List<L> list, int left, int right) {
        if (left < right) {
            int partitionIndex = partitionDec(list, left, right);
            quickDec(list, left, (partitionIndex - 1));
            quickDec(list, (partitionIndex +1), right);
        }
    }
     ...
     * @param       right the value in the list must be greater than a {@code left} parameter.
     * @param       functional lambda expression for comparison.
     * @see         mz.QuickInterface#partition(List, int, int, SortFunctional)
     */
    default <L extends T> void quick(List<L> list, int left, int right, SortFunctional<T> functional) {
        if (left < right) {
            int partitionIndex = partition(list, left, right, functional);
            quick(list, left, (partitionIndex - 1), functional);
            quick(list, (partitionIndex +1), right, functional);
        }
    }
     ...
     */
    @SuppressWarnings("unchecked")
    default <L extends T> int partitionInc(List<L> list, int left, int right) {
     ...
     */
    @SuppressWarnings("unchecked")
    default <L extends T> int partitionDec(List<L> list, int left, int right) {
        L pivot = list.get(right);
        int i = (left - 1);
        for (int j = left; j <= (right - 1); j++) {
            if (pivot.compareTo(list.get(j)) < 0) {
    ...
    default <L extends T> int partition(List<L> list, int left, int right, SortFunctional<T> functional) {
        ...
        int i = (left - 1);
        for (int j = left; j <= (right - 1); j++) {
            if (functional.functionalCompareTo(pivot, list.get(j))) {
                swap(list, ++i, j);
            }
        }
        swap(list, (i + 1), right);
     ...
     * @param       left the value in the list must be smaller than a {@code right} parameter.
     * @param       right the value in the list must be greater than a {@code left} parameter.
     * @return      the pivots in the partitioned list.
     * @see         mz.SortSwap#swap(List, int, int)
     */
    @SuppressWarnings("unchecked")
    default <L extends T> int[] partitionDualInc(List<L> list, int left, int right) {
        if (list.get(left).compareTo(list.get(right)) > 0) {
            swap(list, left, right);
        }
        int newLeft = (left + 1), newRight = (right - 1), selectIndex = (left + 1);
        L pivot1 = list.get(left), pivot2 = list.get(right);
        while(selectIndex <= newRight) {
            if(pivot1.compareTo(list.get(selectIndex)) > 0) {
                swap(list, selectIndex, newLeft++);
            } else if (list.get(selectIndex).compareTo(pivot2) >= 0) {
                while ((list.get(newRight).compareTo(pivot2) > 0) && (selectIndex < newRight)) {
                    newRight--;
                }
                swap(list, selectIndex, newRight--);
                if (pivot1.compareTo(list.get(selectIndex)) > 0) {
                    swap(list, selectIndex, newLeft++);
                }
            }
        ...
        swap(list, left, --newLeft);
        swap(list, right, ++newRight);
        return new int[] {newLeft, newRight};
    }
     ...
     */
    @SuppressWarnings("unchecked")
    default <L extends T> int[] partitionDualDec(List<L> list, int left, int right) {
        if (list.get(left).compareTo(list.get(right)) < 0) {
            swap(list, left, right);
        }
        int newLeft = (left + 1), newRight = (right - 1), selectIndex = (left + 1);
    ...
    /**
     * {@code partitionDual} method takes a list {@code list} of type {@code T},
     * two integer parameters {@code left} and {@code right},
     * and an instance of {@code mz.SortFunctional<T>} as parameters.
     * <ul>
     *     <li>It starts by checking if the element at index {@code left} is considered greater than
     ...
     * @see         mz.SortSwap#swap(List, int, int)
     */
    default <L extends T> int[] partitionDual(List<L> list, int left, int right, SortFunctional<T> functional) {
        ...
        SortFunctional<T> functionalAddEquals = functionalComparableToAddEquals(functional);
        while(selectIndex <= newRight) {
            if(functional.functionalCompareTo(pivot1, list.get(selectIndex))) {
                swap(list, selectIndex, newLeft++);
            } else if (functionalAddEquals.functionalCompareTo(list.get(selectIndex), pivot2)) {
                while ((functional.functionalCompareTo(list.get(newRight), pivot2)) && (selectIndex < newRight)) {
                    newRight--;
                }
                swap(list, selectIndex, newRight--);
                if (functional.functionalCompareTo(pivot1, list.get(selectIndex))) {
