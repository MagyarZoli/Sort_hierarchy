...
import mz.Sort;
import mz.intro.IntroDoubleSelection;

/**
 * IntroDPQ (Dual Pivot Quick) Double Selection Sort is a hybrid sorting algorithm that combines the strengths of Dual Pivot Quick Sort and Double Selection Sort.
 ...
 * To mitigate the risk of Dual Pivot Quick Sort's worst-case behavior, Double Selection Sort monitors the recursion depth during the sorting process.
 ...
 * Additionally, for small subarrays, Intro switches to Double Selection Sort, which has good performance for small input sizes.
 * @since       1.2
...
implements IntroDPQ<Comparable> {
     ...
     * If the depth exceeds a certain threshold, the algorithm switches to HeapSort,
     * which guarantees worst-case <em>O(n log(n))</em> time complexity but has higher overhead.
     * Additionally, for small subarrays, Intro switches to Double Selection Sort, which has good performance for small input sizes.<br><br>
     * <b>Example:</b>
     * <ol>
    ...
    public Sort sortThreadClass() {
        return new IntroDPQDoubleSelection();
    }
}