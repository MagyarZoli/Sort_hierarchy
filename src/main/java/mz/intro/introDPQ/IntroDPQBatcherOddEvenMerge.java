...
/**
 * IntroDPQ (Dual Pivot Quick) Batcher's Odd-even Merge Sort is a hybrid sorting algorithm that combines the strengths of Dual Pivot Quick Sort and Batcher's Odd-even Merge Sort.
 ...
 * To mitigate the risk of Dual Pivot Quick Sort's worst-case behavior, Batcher's Odd-even Merge Sort monitors the recursion depth during the sorting process.
...
implements IntroDPQ<Comparable>{
     ...
     * Is a hybrid sorting algorithm that combines the strengths of Dual Pivot Quick Sort and Batcher's Odd-even Merge Sort.
     * It aims to provide fast average-case performance while maintaining worst-case guarantees.
     * The basic idea behind Intro Sort is to start with Dual Pivot Quick Sort, which is known for its efficiency on average,
     ...
     * If the depth exceeds a certain threshold, the algorithm switches to HeapSort,
     * which guarantees worst-case <em>O(n log(n))</em> time complexity but has higher overhead.
     * Additionally, for small subarrays, Intro switches to Batcher's Odd-even Merge Sort, which has good performance for small input sizes.<br><br>
     * <b>Example:</b>
     * <ol>
    ...
    public Sort sortThreadClass() {
        return new IntroDPQBatcherOddEvenMerge();
    }
}