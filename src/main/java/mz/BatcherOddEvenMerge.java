...
extends OddEvenMerge {
     ...
     * the entire list is considered as the subarray to be sorted.
     * The method then proceeds to sort the entire list using the odd-even merge sort algorithm.
     * @param       list to be arranged.
     * @see         mz.BatcherOddEvenMerge#batcherOddEvenMergeInc(List, int, int)
     */
     ...
     * The method then proceeds to sort the entire list using the odd-even merge sort algorithm.
     * @param       list to be arranged.
     * @see         mz.BatcherOddEvenMerge#batcherOddEvenMergeDec(List, int, int)
     */
     ...
     * {@code batcherOddEvenMerge} that implements the odd-even merge sort algorithm using the batcher's merge algorithm.
     * <ul>
     *     <li>The {@code batcherOddEvenMerge} method takes an list {@code list} of type {@link java.lang.Comparable Comparable}
     *     and an instance of {@code SortFunctional<Comparable>} as parameters.</li>
     ...
     * </ul>
     * {@code batcherOddEvenMerge} this method is to initiate the batcher odd-even merge sorting algorithm.
     * It checks if the list has more than one element and then proceeds to call the main sorting method, passing the necessary parameters.
     * @param       list to be arranged.
     * @param       functional lambda expression for comparison.
     * @see         mz.BatcherOddEvenMerge#batcherOddEvenMerge(List, int, int, SortFunctional)
     */
     ...
     * @see         mz.BatcherOddEvenMerge#batcherMergingInc(List, int, int, int)
     */
     ...
     * @param       left specific range of the list.
     * @param       right specific range of the list.
     * @see         mz.BatcherOddEvenMerge#batcherMergingDec(List, int, int, int)
     */
     ...
     * @param       right specific range of the list.
     * @param       functional lambda expression for comparison.
     * @see         mz.BatcherOddEvenMerge#batcherMerging(List, int, int, int, SortFunctional)
     */
    protected <L extends Comparable> void batcherOddEvenMerge(List<L> list, int left, int right, SortFunctional<Comparable> functional) {
        int n = (right - left);
        if (n <= 1) {
            return;
        }
     ...
     *     It also initializes variables {@code i} and {@code j} to {@code left} and {@code mid}, respectively, and {@code k} to <i>0</i>.</li>
     *     <li>It creates a new {@link java.util.ArrayList ArrayList} called {@code merged} to store the merged elements.</li>
     *     <li>It enters a while loop that continues as long as {@code i} is less than {@code mid} and {@code j} is less than {@code right}.</li>
     ...
     *     Otherwise, it adds the element at index {@code j} to the {@code merged} list and increments {@code j} and {@code k}.</li>
     *     <li>It enters two separate while loops to handle the remaining elements in the first and second halves of the sublist.
     ...
     *     <li>It updates the original {@code list} by setting the elements from indices {@code left} to {@code (left + n - 1)}
     ...
     *     <li>Inside the loop, it compares the element at index {@code m} with the element at index {@code (m + 1)} in the {@code list}.
     *     If the element at index {@code m} is greater than the element at index {@code (m + 1)},
     *     it swaps them using the {@code swap} method.</li>
     * </ul>
     * {@code batcherMergingInc} performs an incremental merging operation on a sublist of the {@code list} between the indices {@code left} and {@code right}.
     * It merges the two halves of the sublist into a new {@code merged} list, updates the original {@code list} with the merged elements,
     ...
     *     it swaps them using the {@code swap} method.</li>
     * </ul>
     * {@code batcherMergingDec} performs a decremental merging operation on a sublist of the {@code list} between the indices {@code left} and {@code right}.
     * It merges the two halves of the sublist into a new {@code merged} list, updates the original {@code list} with the merged elements,
     * and performs a final sorting pass to ensure the odd-even ordering of adjacent elements.
     * @param       list to be arranged.
     * @param       left specific range of the list.
     ...
     *     by calling a method {@code functionalComparableToAddEquals} with the {@code functional} object as an argument.</li>
     *     <li>It enters a while loop that continues as long as {@code i} is less than {@code mid} and {@code j} is less than {@code right}.</li>
     *     <li>Inside the loop, it performs a comparison between the elements at indices i and j in the list using the functionalAddEquals object.
     ...
     *     it swaps them using the {@code swap} method.</li>
     * </ul>
     * {@code batcherMergingInc} performs the merging operation on a sublist of the {@code list} between the indices {@code left} and {@code right}
     ...
     * and performs a final sorting pass to ensure the ordering of adjacent elements using the {@code functional} object for comparisons.
     * @param       list to be arranged.
     * @param       left specific range of the list.
     * @param       mid the middle value of the specified range of the list.
     * @param       right specific range of the list.
     ...
     * @see         mz.SortSwap#swap(List, int, int)
     */
    protected <L extends Comparable> void batcherMerging(List<L> list, int left, int mid, int right, SortFunctional<Comparable> functional) {
        int n = (right - left), i = left, j = mid, k = 0;
        List<L> merged = new ArrayList<>();
        SortFunctional<Comparable> functionalAddEquals = functionalComparableToAddEquals(functional);
        while (i < mid && j < right) {
