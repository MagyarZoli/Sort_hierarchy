...
public interface MergeInterface<T extends Comparable>
    ...
    /**
     * {@code mergeInc} that performs a Merge Sort on an array of {@link java.lang.Comparable Comparable} objects in
     * increasing order.
     ...
     *     <li>Inside the condition, the method recursively calls itself to sort the left and right halves of the
     *     portion.</li>
     ...
     *     the right half of the portion by updating the {@code mid} index to be the midpoint between {@code (mid + 1)}
     *     and {@code right}.</li>
     *     <li>After the recursive calls, the {@code mergingInc} method is called to merge the two sorted halves
     *     together.
     *     The {@code mergingInc} method takes the {@code array}, {@code left}, {@code mid}, and {@code right} indices
     *     as arguments.</li>
     *     <li>The overall effect of the {@code mergeInc} method is to recursively divide the array into smaller
     *     subarrays until they consist of individual elements.
     *     Then, it merges and sorts these individual elements to gradually build the sorted array.</li>
     * </ul>
     * {@code mergeInc} this implementation follows the divide-and-conquer strategy of merge sort, where the array is
     * recursively divided into smaller subarrays,
     * sorted individually, and then merged back together.
     * @param       array to be arranged.
     * @see         MergeInterface#mergeInc(Comparable[], int, int, int)
     */
    ...
    /**
     * {@code mergeDec} that performs a Merge Sort on an array of {@link java.lang.Comparable Comparable} objects in
     * decreasing order.
     * The method recursively divides the array into smaller subarrays, sorts them individually,
     ...
     *     the right half of the portion by updating the {@code mid} index to be the midpoint between {@code (mid + 1)}
     *     and {@code right}.</li>
     *     <li>After the recursive calls, the {@code mergingDec} method is called to merge the two sorted halves
     *     together.
     *     The {@code mergingDec} method takes the {@code array}, {@code left}, {@code mid}, and {@code right} indices
     *     as arguments.</li>
     *     <li>The overall effect of the {@code mergeDec} method is to recursively divide the array into greater
     *     subarrays until they consist of individual elements.
     *     Then, it merges and sorts these individual elements to gradually build the sorted array.</li>
     * </ul>
     * {@code mergeDec} this implementation follows the divide-and-conquer strategy of merge sort, where the array is
     * recursively divided into smaller subarrays,
     * sorted individually, and then merged back together.
     * @param       array to be arranged.
     * @see         MergeInterface#mergeDec(Comparable[], int, int, int)
     */
     ...
     *     <li>The third recursive call is made to the {@code merging} method with the original indices <i>0</i>,
     *     {@code (array.length / 2)}, and {@code array.length}.
     ...
     *     <li>The recursion continues until the base case is reached, where <i>0</i> is no longer less than
     *     {@code array.length}.
     *     At this point, the recursion unwinds,
     *     and the merge operations are performed for each level of recursion until the original array is completely
     *     sorted.</li>
     * </ul>
     * merge two sorted subarrays into a single sorted array according to the intended sorting logic.
     * @param       array to be arranged.
     * @param       functional lambda expression for comparison.
     * @see         MergeInterface#merge(Comparable[], int, int, int, SortFunctional)
     */
     ...
     * <ul>
     *     <li>The method takes an array of {@code Comparable} objects, denoted by {@code array},
     *     and the indices {@code left}, {@code mid}, and {@code right} that define the portion of the array to be
     *     sorted.</li>
     *     <li>The condition {@code if (left < right)} checks if the portion of the array has more than one element.
     *     If so, it proceeds with the merge sort algorithm.</li>
     *     <li>Inside the condition, the method recursively calls itself to sort the left and right halves of the
     ...
     * @see         MergeInterface#mergingInc(Comparable[], int, int, int)
     */
     ...
     *                  than the {@code left}.
     * @param       right from the array, index value, must be greater than the {@code left} and {@code mid} value.
     * @see         MergeInterface#mergingDec(Comparable[], int, int, int)
     */
     ...
     *     which splits the left half of the array recursively.</li>
     *     <li>The second recursive call is made with the indices {@code (mid + 1)}, {@code (((mid + 1) + right) / 2)},
     *     {@code right},
     *     which splits the right half of the array recursively.</li>
     *     <li>The third recursive call is made to the {@code merging} method with the original indices {@code left},
     *     {@code mid}, and {@code right}.
     *     This is the merge step where two sorted subarrays (from {@code left} to {@code mid} and from
     *     {@code (mid + 1)} to {@code right})
     *     are merged into a single sorted array using the provided sorting logic.</li>
     *     <li>The recursion continues until the base case is reached, where {@code left} is no longer less than
     *     {@code right}.
     *     At this point, the recursion unwinds,
     *     and the merge operations are performed for each level of recursion until the original array is completely
     *     sorted.</li>
     ...
     * @param       right from the array, index value, must be greater than the {@code left} and {@code mid} value.
     * @param       functional lambda expression for comparison.
     * @see         MergeInterface#merging(Comparable[], int, int, int, SortFunctional)
     */
     ...
     * <ul>
     *     <li>This {@code while} loop iterates until the {@code left} index becomes equal to or greater than the
     *     {@code right} index,
     ...
     *     <li>Inside the {@code while} loop, there is a {@code for} loop that iterates over the array segments with
     *     a size of {@code (left * 2)}.
     ...
     *     the variables {@code mid} and {@code end} are calculated to determine the indices for the merging
     *     operation.</li>
     ...
     *     The merging method performs the merge operation for the specified array segment using the provided sorting
     *     logic.</li>
     *     <li>After the {@code for} loop completes, the {@code left} value is multiplied by 2 to double its value,
     *     preparing for the next iteration.
     *     This step ensures that the array segments to be merged in the next iteration are twice as large as in the
     ...
     * @see         MergeInterface#mergingInc(Comparable[], int, int, int, Comparable[])
     */
     ...
     * @param       right The ending index (exclusive) of the subarray to be sorted.
     * @param       buffer An auxiliary array used for merging.
     * @see         MergeInterface#mergingDec(Comparable[], int, int, int, Comparable[])
     */
     ...
     *     <li>Within the {@code while} loop, there is a {@code for} loop that iterates over subarrays of size
     *     {@code (left * 2)}.
     ...
     *     indicating that the entire array has been sorted.</li>
     * </ul>
     * {@code merge} method implements a bottom-up merge sort algorithm by repeatedly merging subarrays of increasing
     * sizes.
     * It uses the {@code merging} method to perform the merging operation.
     * The method starts with a merge size of {@code left} and doubles it in each iteration until the entire array is
     * sorted.
     * @param       array The array to be sorted.
     * @param       left The starting index of the subarray to be sorted.
     * @param       right The ending index (exclusive) of the subarray to be sorted.
     * @param       buffer An auxiliary array used for merging.
     * @param       functional lambda expression for comparison.
     * @see         MergeInterface#merging(Comparable[], int, int, int, Comparable[], SortFunctional)
     */
    ...
    /**
     * {@code mergeInc} method for merging three sorted subarrays into a single array. It takes the indices of the
     * subarray boundaries
     * and uses four pointers {@code i, j, k, l} to iterate through the subarrays and merge them into a buffer array.
     * <ul>
     *     <li>{@link java.lang.SuppressWarnings @SuppressWarnings}{@code ("unchecked")}: This annotation suppresses
     *     unchecked warnings that might occur when using generics,
     ...
     *     This nested condition checks if the element at position {@code i} in the first subarray is also less than
     *     the element
     *     at position {@code k} in the third subarray.</li>
     *     <li> If both conditions in steps 5 and 6 are true, the element at position {@code i} in the first subarray is
     *     copied to
     *     the {@code buffer} array at position {@code l}.
     *     Then, both {@code i} and {@code l} pointers are incremented.</li>
     *     <li>If the condition in step 6 is false, the element at position {@code k} in the third subarray is selected
     *     for merging.</li>
     *     <li>The element at position {@code k} in the third subarray is copied to the {@code buffer} array at position
     *     {@code l}.
     *     Then, both {@code k} and {@code l} pointers are incremented.</li>
     *     <li>If the condition in step 5 is false, the element at position {@code j} in the second subarray is selected
     *     for merging.</li>
     *     <li>{@code if (array[j].compareTo(array[k]) < 0)}:
     *     This nested condition checks if the element at position {@code j} in the second subarray is less than the
     *     element
     *     at position {@code k} in the third subarray.</li>
     *     <li>If the condition in step 11 is true, the element at position {@code j} in the second subarray is copied
     *     to the {@code buffer} array at position {@code l}.
     *     Then, both {@code j} and {@code l} pointers are incremented.</li>
     *     <li>If the condition in step 11 is false, the element at position {@code k} in the third subarray is selected
     *     for merging.</li>
     *     <li>The element at position {@code k} in the third subarray is copied to the buffer array at position
     *     {@code l}.
     *     Then, both {@code k} and {@code l} pointers are incremented.</li>
     *     <li>The following three while loops merge any remaining elements
     *     in pairs of subarrays {@code ((i, j), (j, k), (i, k))} into the buffer array, similar to the previous
     *     steps.</li>
     *     <li>The remaining three {@code while} loops copy any remaining elements from the subarrays {@code i, j, k}
     *     to the buffer array.</li>
     * </ul>
     * {@code mergeInc} this method merges three sorted subarrays into a single sorted array using four pointers to
     * iterate through
    ...
    /**
     * {@code mergeDec} method for merging three sorted subarrays into a single array. It takes the indices of the
     * subarray boundaries
     * and uses four pointers {@code i, j, k, l} to iterate through the subarrays and merge them into a buffer array.
     * <ul>
     *     <li>{@link java.lang.SuppressWarnings @SuppressWarnings}{@code ("unchecked")}: This annotation suppresses
     ...
     *     {@code k} points to the current position in the third subarray, and
     *     {@code l} points to the current position in the buffer array.</li>
     *     <li>This {@code while} loop continues as long as all three subarrays have remaining elements to be
     *     processed.</li>
     *     <li>This condition checks if the element at position {@code i} in the first subarray is more than the element
     *     at position {@code j} in the second subarray.</li>
     *     <li>{@code if (array[i].compareTo(array[k]) > 0)}:
     *     This nested condition checks if the element at position {@code i} in the first subarray is also less than the
     *     element
     *     at position {@code k} in the third subarray.</li>
     *     <li> If both conditions in steps 5 and 6 are true, the element at position {@code i} in the first subarray is
     *     copied to
     ...
     *     for merging.</li>
     *     <li>{@code if (array[j].compareTo(array[k]) > 0)}:
     *     This nested condition checks if the element at position {@code j} in the second subarray is more than the
     *     element
     *     at position {@code k} in the third subarray.</li>
     *     <li>If the condition in step 11 is true, the element at position {@code j} in the second subarray is copied
     *     to the {@code buffer} array at position {@code l}.
     ...
     *     in pairs of subarrays {@code ((i, j), (j, k), (i, k))} into the buffer array, similar to the previous
     *     steps.</li>
     *     <li>The remaining three {@code while} loops copy any remaining elements from the subarrays {@code i, j, k} to
     *     the buffer array.</li>
     * </ul>
     * {@code mergeInc} this method merges three sorted subarrays into a single sorted array using four pointers to
     * iterate through
     ...
     *     Within the loop, it performs comparisons using the {@code functional} instance to determine the order of the
     *     elements.</li>
     ...
     * </ul>
     * {@code merge} method performs a three-way merge of subarrays within the {@code array} by comparing elements using
     * the {@code functional} instance.
     * It maintains separate indices for each subarray and incrementally merges the elements into the {@code buffer}
     * array.
    ...
    /**
     * {@code mergingInc} that performs a merging operation for the purpose of merging two sorted portions of an array
     * into a single sorted portion.
     * The method assumes that the left portion of the array, from index {@code left} to {@code mid}, and the
     * {@code right} portion,
     ...
     *     warnings.</li>
     *     <li>The method starts by checking if the indices {@code left}, {@code mid}, and {@code right} are within
     *     valid bounds.
     ...
     *     <li>The variables {@code n1} and {@code n2} are assigned the sizes of the left and right portions,
     *     respectively.</li>
     *     <li>Two new arrays, {@code leftArray} and {@code rightArray}, are created using the
     *     {@link java.util.Arrays#copyOfRange(Object[], int, int) Arrays.copyOfRange} method.
     *     These arrays contain copies of the respective portions of the original {@code array}.</li>
     *     <li>The variables {@code i} and {@code j} are used as indices to iterate over {@code leftArray} and
     *     {@code rightArray}, respectively.</li>
     *     <li>The variable {@code k} is used as an index to update the {@code array} with the merged elements.</li>
     *     <li>The subsequent {@code while} loop continues as long as both {@code i} and {@code j} are within their
     *     respective array sizes, {@code n1} and {@code n2}.
     ...
     *     <li>After the {@code while} loop, there might still be remaining elements in either {@code leftArray} or
     *     {@code rightArray}.
     *     The next two {@code while} loops handle the remaining elements by copying them into array starting from index
     *     {@code k}.</li>
     *     <li>Finally, the merging operation is complete, and the merged sorted portion is stored in the original
     *     {@code array} from index {@code left} to {@code right}.</li>
     * </ul>
     * {@code mergingInc} method is commonly used as part of merge sort algorithms to merge two sorted portions of an
     * array into a single sorted portion.
     * @param       array to be arranged.
     * @param       left from the array, index value, must be smaller than the {@code mid} and {@code right} value.
     * @param       mid from the array, index value, must be smaller than the {@code right} value, and must be greater
     *                  than the {@code left}.
    ...
    /**
     * {@code mergingDec} that performs a merging operation for the purpose of merging two sorted portions of an array
     * into a single sorted portion.
     * The method assumes that the left portion of the array, from index {@code left} to {@code mid}, and the
     * {@code right} portion,
     * from index {@code (mid + 1)} to {@code right}, are already sorted in increasing order.
     ...
     *     <li>After the {@code while} loop, there might still be remaining elements in either {@code leftArray} or
     *     {@code rightArray}.
     *     The next two {@code while} loops handle the remaining elements by copying them into array starting from
     *     index {@code k}.</li>
     *     <li>Finally, the merging operation is complete, and the merged sorted portion is stored in the original
     *     {@code array} from index {@code left} to {@code right}.</li>
     * </ul>
     * {@code mergingDec} method is commonly used as part of merge sort algorithms to merge two sorted portions of an
     * array into a single sorted portion.
     * @param       array to be arranged.
     * @param       left from the array, index value, must be smaller than the {@code mid} and {@code right} value.
     * @param       mid from the array, index value, must be smaller than the {@code right} value, and must be greater
    ...
    /**
     * {@code merging} that performs a merging operation for the purpose of merging two sorted portions of an array into
     * a single sorted portion.
     * The method assumes that the left portion of the array, from index {@code left} to {@code mid}, and the
     * {@code right} portion,
     ...
     *     <li>It initializes variables {@code n1} and {@code n2} to store the sizes of the left and right subarrays,
     *     respectively.
     *     The values are calculated as {@code (mid - left + 1)} and {@code (right - mid)}.</li>
     *     <li>It initializes counters {@code i}, {@code j}, and {@code k} to keep track of the current indices in the
     ...
     *     <li>It enters a {@code while} loop that continues as long as both {@code i} and {@code j} are within their
     *     respective
     ...
     *     After assigning the element, it increments the corresponding counters ({@code i}, {@code j}, and
     *     {@code k}).</li>
     *     <li>After exiting the {@code while} loop, it checks if there are any remaining elements in the left subarray
     *     {@code (i < n1)}.
     ...
     * </ul>
     * {@code merging} method merges two sorted subarrays ({@code leftArray} and {@code rightArray}) into a single
     * sorted array ({@code array}).
     * It compares elements using the {@code functional} instance and assigns the smaller element to {@code array[k]} at
     * each step.
     ...
     *     {@code n1} is initially set to the leftmost index, and {@code n2} is set to the index immediately after the
     *     midpoint.</li>
     *     <li>This loop iterates from the {@code left} index to the {@code right} index (inclusive) to merge the
     *     subarrays.</li>
     *     <li>This condition checks if the current element at position {@code n1} in the left subarray is less than or
     *     equal to
     ...
     *     <li>If the condition in step 5 is false, the element at position {@code n2} in the right subarray is selected
     *     for merging.</li>
     *     <li>The element at position {@code n2} in the right subarray is copied to the {@code buffer} array at
     *     position {@code i}.
     ...
     * </ul>
     * {@code mergingInc} this method performs the merging step of the merge sort algorithm by comparing elements from
     * two sorted subarrays
     * and merging them into a temporary buffer array.
     * After the merging is complete, the merged elements are copied back
     * {@link java.lang.System#arraycopy(Object, int, Object, int, int) System.arraycopy} to the original array.
     * @param       array to be arranged.
     ...
     *     <li>This loop iterates from the {@code left} index to the {@code right} index (inclusive) to merge the
     *     subarrays.</li>
     *     <li>This condition checks if the current element at position {@code n1} in the left subarray is more than or
     *     equal to
     *     the current element at position {@code n2} in the right subarray.
     *     It also checks if the {@code n2} pointer has reached the end of the right subarray.
     *     If either of these conditions is true, the element at {@code n1} is selected for merging.</li>
     ...
     *     The number of elements to copy is {@code (right - left + 1)}.</li>
     * </ul>
     * {@code mergingDec} this method performs the merging step of the merge sort algorithm by comparing elements from
     * two sorted subarrays
     * and merging them into a temporary buffer array.
     * After the merging is complete, the merged elements are copied back
     * {@link java.lang.System#arraycopy(Object, int, Object, int, int) System.arraycopy} to the original array.
     ...
     *     <li>It creates a {@code functionalAddEquals} instance using the {@code functionalComparableToAddEquals}
     *     method, which is responsible for defining the comparison logic for determining element order.</li>
     ...
     *     <li>Otherwise, it assigns the element from the right subarray to {@code buffer[i]} and increments
     *     {@code n2}.</li>
     *     <li>After the loop, it uses {@link java.lang.System#arraycopy(Object, int, Object, int, int)
     *     System.arraycopy} to copy the elements from {@code buffer} back to the original {@code array},
     *     starting from the {@code left} index and copying {@code (right - left + 1)} elements.</li>
     * </ul>
     * {@code merging} method merges two sorted subarrays, specified by the indices {@code left}, {@code mid}, and
     * {@code right}, into a single sorted array {@code array}.
     ...
     *     unchecked warnings that might occur when using generics,
     *     indicating that the code will handle any necessary type checking appropriately.</li>
     *     <li>Initializes three pointers {@code i, j, k}. {@code i} points to the current position in the left
     *     subarray, {@code j} points to the current position in the right subarray, and {@code k} points to the current
     *     position in the merged array.</li>
     *     <li>This {@code while} loop continues as long as both {@code i} and {@code j} are within the bounds of their
     *     respective subarrays.</li>
     *     <li>{@code if (leftArray[i].compareTo(rightArray[j]) < 0)}:
     *     This condition checks if the element at position {@code i} in the left subarray is less than the element at
     *     position {@code j} in the right subarray.
     *     If true, the element from the left subarray is selected for merging.</li>
     *     <li>If the condition in the previous step is true, the element at position {@code i} in the left subarray is
     *     copied to the {@code array} at position {@code k}.
     *     Then, all three pointers {@code i, j, k} are incremented.</li>
     *     <li>If the condition in step 5 is false, the element at position {@code j} in the right subarray is selected
     *     for merging.</li>
     *     <li>The element at position {@code j} in the right subarray is copied to the {@code array} at position
     *     {@code k}.
     *     Then, all three pointers {@code i, j, k} are incremented.</li>
     *     <li>This {@code while} loop copies any remaining elements in the left subarray to the {@code array} if there
     *     are any.</li>
     *     <li>The remaining element at position {@code i} in the left subarray is copied to the {@code array} at
     *     position {@code k}.
     ...
     *     <li>The remaining element at position {@code j} in the right subarray is copied to the {@code array} at
     *     position {@code k}.
     *     Then, both {@code j} and {@code k} pointers are incremented.</li>
     * </ul>
     * {@code mergingInc} this method merges two sorted subarrays {@code leftArray} and {@code rightArray} into a single
     * sorted array {@code array}.
     * It compares elements from both subarrays and copies them to the {@code array} in the correct order until one of
     * the subarrays is fully processed.
     ...
     *     <li>Initializes three pointers {@code i, j, k}. {@code i} points to the current position in the left
     *     subarray,
     *     {@code j} points to the current position in the right subarray, and {@code k} points to the current position
     *     in the merged array.</li>
     *     <li>This {@code while} loop continues as long as both {@code i} and {@code j} are within the bounds of their
     *     respective subarrays.</li>
     *     <li>{@code if (leftArray[i].compareTo(rightArray[j]) > 0)}:
     *     This condition checks if the element at position {@code i} in the left subarray is more than the element at
     *     position {@code j} in the right subarray.
     *     If true, the element from the left subarray is selected for merging.</li>
     *     <li>If the condition in the previous step is true, the element at position {@code i} in the left subarray is
     *     copied to the {@code array} at position {@code k}.
     ...
     *     Then, both {@code j} and {@code k} pointers are incremented.</li>
     * </ul>
     * {@code mergingDec} this method merges two sorted subarrays {@code leftArray} and {@code rightArray} into a single
     * sorted array {@code array}.
     * It compares elements from both subarrays and copies them to the {@code array} in the correct order until one of
     * the subarrays is fully processed.
     * Then, it copies any remaining elements from the unfinished subarray to the array.
     ...
     *     <li>Within the loop, it compares the current elements at indices i and j from leftArray and rightArray,
     *     respectively.
     *     If the comparison indicates that the element in {@code rightArray} is considered greater than the element in
     *     {@code leftArray},
     *     it assigns the element from {@code leftArray} to {@code array[k]} and increments {@code i} and {@code k}.
     *     Otherwise, it assigns the element from {@code rightArray} to {@code array[k]} and increments {@code j} and
     *     {@code k}.</li>
     *     <li>After the loop, it checks if there are any remaining elements in {@code leftArray} and {@code rightArray}
     ...
     * {@code merging} method merges two sorted subarrays, {@code leftArray} and {@code rightArray}, into a single
     * sorted array {@code array}.
     * It uses the comparison logic defined by the {@code mz.SortFunctional} instance to determine the order of elements
    ...
    /**
     * {@code mergeInc} that performs a Merge Sort on a list of {@link java.lang.Comparable Comparable} objects in
     * increasing order.
     * The method recursively divides the list into smaller sublists, sorts them individually,
     * and then merges them together using the {@code mergingInc} method.
     * <ul>
     ...
     *     <li>After the recursive calls, the {@code mergingInc} method is called to merge the two sorted halves
     *     together.
     *     The {@code mergingInc} method takes the {@code list}, {@code left}, {@code mid}, and {@code right} indices as
     *     arguments.</li>
     *     <li>The overall effect of the {@code mergeInc} method is to recursively divide the list into smaller
     *     sublists until they consist of individual elements.
     *     Then, it merges and sorts these individual elements to gradually build the sorted list.</li>
     * </ul>
     * {@code mergeInc} this implementation follows the divide-and-conquer strategy of merge sort, where the list is
     * recursively divided into smaller sublists,
     * sorted individually, and then merged back together.
     * @param       list to be arranged.
     * @see         MergeInterface#mergeInc(List, int, int, int)
     */
    ...
    /**
     * {@code mergeDec} that performs a Merge Sort on a list of {@link java.lang.Comparable Comparable} objects in
     * decreasing order.
     * The method recursively divides the list into smaller sublists, sorts them individually,
     * and then merges them together using the {@code mergingDec} method.
     * <ul>
     ...
     *     <li>After the recursive calls, the {@code mergingDec} method is called to merge the two sorted halves
     *     together.
     *     The {@code mergingDec} method takes the {@code list}, {@code left}, {@code mid}, and {@code right} indices as
     *     arguments.</li>
     *     <li>The overall effect of the {@code mergeDec} method is to recursively divide the list into greater sublists
     *     until they consist of individual elements.
     *     Then, it merges and sorts these individual elements to gradually build the sorted list.</li>
     * </ul>
     * {@code mergeDec} this implementation follows the divide-and-conquer strategy of merge sort, where the list is
     * recursively divided into smaller sublists,
     * sorted individually, and then merged back together.
     * @param       list to be arranged.
     * @see         MergeInterface#mergeDec(List, int, int, int)
     */
     ...
     *     <li>The third recursive call is made to the {@code merging} method with the original indices <i>0</i>,
     *     {@code (list.size() / 2)}, and {@code list.length}.
     *     This is the merge step where two sorted sublists (from <i>0</i> to {@code (list.size() / 2)}
     *     and from {@code ((list.size() / 2) + 1)} to {@code list.size()})
     *     are merged into a single sorted list using the provided sorting logic.</li>
     *     <li>The recursion continues until the base case is reached, where <i>0</i> is no longer less than {
     *     @code list.size()}.
     *     At this point, the recursion unwinds,
     *     and the merge operations are performed for each level of recursion until the original list is completely
     *     sorted.</li>
     * </ul>
     * merge two sorted sublists into a single sorted list according to the intended sorting logic.
     * @param       list to be arranged.
     * @param       functional lambda expression for comparison.
     ...
     * <ul>
     *     <li>The method takes an list of {@code Comparable} objects, denoted by {@code list},
     *     and the indices {@code left}, {@code mid}, and {@code right} that define the portion of the list to be
     *     sorted.</li>
     *     <li>The condition {@code if (left < right)} checks if the portion of the list has more than one element.
     *     If so, it proceeds with the merge sort algorithm.</li>
     *     <li>Inside the condition, the method recursively calls itself to sort the left and right halves of the
     ...
     *     The {@code mergingInc} method takes the {@code list}, {@code left}, {@code mid}, and {@code right} indices as
     *     arguments.</li>
     *     <li>The overall effect of the {@code mergeInc} method is to recursively divide the list into smaller sublists
     *     until they consist of individual elements.
     *     Then, it merges and sorts these individual elements to gradually build the sorted list.</li>
     * </ul>
     * {@code mergeInc} this implementation follows the divide-and-conquer strategy of merge sort, where the list is
     ...
     * @see         MergeInterface#mergingInc(List, int, int, int)
     */
     ...
     *              than the {@code left}.
     * @param       right from the list, index value, must be greater than the {@code left} and {@code mid} value.
     * @see         MergeInterface#mergingDec(List, int, int, int)
     */
     ...
     *     <li>The third recursive call is made to the {@code merging} method with the original indices {@code left},
     *     {@code mid}, and {@code right}.
     *     This is the merge step where two sorted sublists (from {@code left} to {@code mid} and from {@code (mid + 1)}
     *     to {@code right})
     *     are merged into a single sorted list using the provided sorting logic.</li>
     *     <li>The recursion continues until the base case is reached, where {@code left} is no longer less than
     *     {@code right}.
     ...
     * @param       right from the list, index value, must be greater than the {@code left} and {@code mid} value.
     * @param       functional lambda expression for comparison.
     * @see         MergeInterface#merging(List, int, int, int, SortFunctional)
     */
     ...
     *     effectively dividing the list into sublists of size {@code (left * 2)} for merging.</li>
     *     <li>This calculates the indices for the merging operation.
     *     {@code mid} represents the midpoint of the current sublists, and
     *     {@code end} represents the ending index of the current sublists or the ending index of the entire list
     *     if it's reached before the sublists size.</li>
     *     <li>This calls the {@code mergingInc} method to merge the sublists defined by the indices
     *     {@code i}, {@code mid}, and {@code end}.</li>
     *     <li>{@code left *= 2} After each iteration of the outer loop, the {@code left} variable is doubled.
     *     This increases the size of the sublists being merged in the next iteration.</li>
     * </ul>
     * {@code mergeInc} method performs an iterative bottom-up merge sort by repeatedly merging sublists of
     * increasing sizes until the entire list is sorted.
     * It utilizes the {@code mergingInc} method to perform the merging operation.
     * @param       list The list to be sorted.
     * @param       left The starting index of the sublists to be sorted.
     * @param       right The ending index (exclusive) of the sublists to be sorted.
     * @see         MergeInterface#mergingInc(List, int, int, int)
     */
     ...
     *     {@code end} represents the ending index of the current sublists or the ending index of the entire list
     *     if it's reached before the sublists size.</li>
     *     <li>This calls the {@code mergingDec} method to merge the sublists defined by the indices
     *     {@code i}, {@code mid}, and {@code end}.</li>
     *     <li>{@code left *= 2} After each iteration of the outer loop, the {@code left} variable is doubled.
     *     This decreases the size of the sublists being merged in the next iteration.</li>
     * </ul>
     * {@code mergeDec} method performs an iterative bottom-up merge sort by repeatedly merging sublists of
     * increasing sizes until the entire list is sorted.
     * It utilizes the {@code mergingDec} method to perform the merging operation.
     * @param       list The list to be sorted.
     * @param       left The starting index of the sublists to be sorted.
    ...
    /**
     * {@code merge} method merge takes a list {@code list}, indices {@code left} and {@code right}, and a
     * {@code SortFunctional} as parameters that defines the sorting logic.
     * <ul>
     *     <li>The {@code while} loop continues until the {@code left} index is less than the {@code right} index.</li>
     *     <li>Inside the {@code while} loop, there is a {@code for} loop that iterates over the list segments with a
     *     size of {@code (left * 2)}.
     *     This means that in each iteration of the {@code for} loop,
     ...
     *     the indices {@code i}, {@code mid}, {@code end}, and the {@code functional} as parameters.
     *     The merging method performs the merge operation for the specified list segment using the provided sorting
     *     logic.</li>
     *     <li>After the {@code for} loop completes, the {@code left} value is multiplied by 2 to double its value,
     *     preparing for the next iteration.
     *     This step ensures that the list segments to be merged in the next iteration are twice as large as in the
     *     previous iteration.</li>
     *     <li>The process continues until the {@code left} index is no longer less than the {@code right} index,
     ...
     * @see         MergeInterface#mergingInc(List, int, int, int, List)
     */
     ...
     * @param       right The ending index (exclusive) of the sublists to be sorted.
     * @param       buffer An auxiliary list used for merging.
     * @see         MergeInterface#mergingDec(List, int, int, int, List)
     */
     ...
     * <ul>
     *     <li>The method takes in parameters {@code left} and {@code right},
     *     which represent the boundaries of the sublists to be merged.
     *     It also takes a {@code buffer} list to store the merged elements and
     *     the {@code functional} instance for element comparison.</li>
     *     <li>The method enters a {@code while} loop that continues as long as {@code left} is less than {@code right}.
     *     This loop iterates over multiple merge passes, doubling the merge size {@code left} in each iteration.</li>
     *     <li>Within the {@code while} loop, there is a {@code for} loop that iterates over sublists of size
     *     {@code (left * 2)}.
     *     The loop variable {@code i} represents the starting index of each sublists.</li>
     *     <li>For each sublists, it calculates the {@code mid} index as {@code (i + left - 1)},
     *     which represents the {@code end} of the first half of the sublists.
     *     The end index is calculated as the minimum of {@code (i + (left * 2) - 1)} and {@code (right - 1)},
     *     ensuring that it does not exceed the right boundary of the list.</li>
     *     <li>It calls the {@code merging} method to merge the sublists,
     *     passing the {@code list}, {@code i}, {@code mid}, {@code end}, {@code buffer},
     *     and {@code functional} as parameters.</li>
     *     <li>After merging a sublists, the {@code left} value is doubled {@code (left *= 2)}
     *     to increase the merge size for the next iteration.</li>
     *     <li>The process continues until {@code left} becomes greater than or equal to {@code right},
     *     indicating that the entire list has been sorted.</li>
     * </ul>
     * {@code merge} method implements a bottom-up merge sort algorithm by repeatedly merging sublists of increasing
     * sizes.
     * It uses the {@code merging} method to perform the merging operation.
     * The method starts with a merge size of {@code left} and doubles it in each iteration until the entire list is
     * sorted.
     * @param       list The list to be sorted.
     * @param       left The starting index of the sublists to be sorted.
     * @param       right The ending index (exclusive) of the sublists to be sorted.
     * @param       buffer An auxiliary list used for merging.
     * @param       functional lambda expression for comparison.
     * @see         MergeInterface#merging(List, int, int, int, List, SortFunctional)
     */
    ...
    /**
     * {@code mergeInc} method for merging three sorted sublists into a single list. It takes the indices of the
     * sublists boundaries
     * and uses four pointers {@code i, j, k, l} to iterate through the sublists and merge them into a buffer list.
     * <ul>
     *     <li>{@link java.lang.SuppressWarnings @SuppressWarnings}{@code ("unchecked")}: This annotation suppresses
     *     unchecked warnings that might occur when using generics,
     *     indicating that the code will handle any necessary type checking appropriately.</li>
     *     <li>Initializes four pointers {@code i, j, k, l}.
     *     {@code i} points to the current position in the first sublists,
     *     {@code j} points to the current position in the second sublists,
     *     {@code k} points to the current position in the third sublists, and
     *     {@code l} points to the current position in the buffer list.</li>
     *     <li>This {@code while} loop continues as long as all three sublists have remaining elements to be
     *     processed.</li>
     *     <li>This condition checks if the element at position {@code i} in the first sublists is less than the element
     *     at position {@code j} in the second sublists.</li>
     *     <li>{@code if (list.get(k).compareTo(list.get(i)) > 0)}:
     *     This nested condition checks if the element at position {@code i} in the first subarray is also less than the
     *     element
     *     at position {@code k} in the third sublists.</li>
     *     <li> If both conditions in steps 5 and 6 are true, the element at position {@code i} in the first sublists is
     *     copied to
     *     the {@code buffer} list at position {@code l}.
     *     Then, both {@code i} and {@code l} pointers are incremented.</li>
     *     <li>If the condition in step 6 is false, the element at position {@code k} in the third sublists is selected
     *     for merging.</li>
     *     <li>The element at position {@code k} in the third sublists is copied to the {@code buffer} list at position
     *     {@code l}. Then, both {@code k} and {@code l} pointers are incremented.</li>
     *     <li>If the condition in step 5 is false, the element at position {@code j} in the second sublists is selected
     *     for merging.</li>
     *     <li>{@code if (list.get(k).compareTo(list.get(j)) > 0)}:
     *     This nested condition checks if the element at position {@code j} in the second sublists is less than the
     *     element at position {@code k} in the third sublists.</li>
     *     <li>If the condition in step 11 is true, the element at position {@code j} in the second sublists is copied
     *     to the {@code buffer} list at position {@code l}.
     *     Then, both {@code j} and {@code l} pointers are incremented.</li>
     *     <li>If the condition in step 11 is false, the element at position {@code k} in the third sublists is selected
     *     for merging.</li>
     *     <li>The element at position {@code k} in the third sublists is copied to the buffer list at position
     *     {@code l}. Then, both {@code k} and {@code l} pointers are incremented.</li>
     *     <li>The following three while loops merge any remaining elements in pairs of sublists
     *     {@code ((i, j), (j, k), (i, k))} into the buffer list, similar to the previous steps.</li>
     *     <li>The remaining three {@code while} loops copy any remaining elements from the sublists {@code i, j, k} to
     *     the buffer list.</li>
     * </ul>
     * {@code mergeInc} this method merges three sorted sublists into a single sorted list using four pointers to
     * iterate through
     * the sublists and merge their elements into a buffer list.
     ...
     * @param       left The starting index of the first sublists.
     * @param       mid1 The ending index (exclusive) of the first sublists.
     * @param       mid2 The ending index (exclusive) of the second sublists.
     * @param       right The ending index (exclusive) of the third sublists.
     * @param       buffer An auxiliary list used for merging.
     */
    @SuppressWarnings("unchecked")
    ...
    /**
     * {@code mergeDec} method for merging three sorted sublists into a single list. It takes the indices of the sublists boundaries
     * and uses four pointers {@code i, j, k, l} to iterate through the sublists and merge them into a buffer list.
     * <ul>
     *     <li>{@link java.lang.SuppressWarnings @SuppressWarnings}{@code ("unchecked")}: This annotation suppresses
     *     unchecked warnings that might occur when using generics,
     ...
     *     <li>This {@code while} loop continues as long as all three sublists have remaining elements to be
     *     processed.</li>
     *     <li>This condition checks if the element at position {@code i} in the first sublists is more than the element
     *     at position {@code j} in the second sublists.</li>
     *     <li>{@code if (list.get(j).compareTo(list.get(i)) < 0)}:
     *     This nested condition checks if the element at position {@code i} in the first sublists is also less than the
     *     element
     *     at position {@code k} in the third subarray.</li>
     *     <li> If both conditions in steps 5 and 6 are true, the element at position {@code i} in the first sublists is
     *     copied to
     ...
     *     for merging.</li>
     *     <li>{@code if (list.get(k).compareTo(list.get(j)) < 0)}:
     *     This nested condition checks if the element at position {@code j} in the second sublists is more than the
     *     element at position {@code k} in the third sublists.</li>
     *     <li>If the condition in step 11 is true, the element at position {@code j} in the second sublists is copied
     *     to the {@code buffer} list at position {@code l}.
     *     Then, both {@code j} and {@code l} pointers are incremented.</li>
     ...
     *     Then, both {@code k} and {@code l} pointers are incremented.</li>
     *     <li>The following three while loops merge any remaining elements
     *     in pairs of sublists {@code ((i, j), (j, k), (i, k))} into the buffer list, similar to the previous
     *     steps.</li>
     *     <li>The remaining three {@code while} loops copy any remaining elements from the sublists {@code i, j, k}
     *     to the buffer list.</li>
     * </ul>
     * {@code mergeInc} this method merges three sorted sublists into a single sorted list using four pointers to
     * iterate through
     * the sublists and merge their elements into a buffer list.
    ...
    /**
     * {@code mergeDec} method for merging three sorted sublists into a single list. It takes the indices of the
     * sublists boundaries
     * and uses four pointers {@code i, j, k, l} and an instance of {@code SortFunctional<T>} as parameters,
     * to iterate through the sublists and merge them into a buffer list.
     * <ul>
     *     <li>The method takes in additional parameters {@code mid1} and {@code mid2},
     *     which represent the boundaries of the two middle sublists.</li>
     *     <li>It initializes variables {@code i, j, k, l} to keep track of
     *     the current indices in the three sublists and the merged list ({@code buffer}), respectively.
     *     The initial value of {@code l} is set to {@code left}.</li>
     *     <li>It enters a {@code while} loop that continues as long as {@code i} is less than {@code mid1}, {@code j}
     *     is less than {@code mid2}, and {@code k} is less than {@code right}.
     *     Within the loop, it performs comparisons using the {@code functional} instance to determine the order of the
     ...
     *     Otherwise, it assigns {@code list.get(k)} to {@code buffer.get(l++)} and increments {@code k}.</li>
     *     <li>After the {@code while} loop, there are three possible cases:</li>
     *     <li>Elements remaining in both the first and second sublists:
     ...
     *     It then increments the corresponding counters {@code i} and {@code j}.</li>
     *     <li>Elements remaining in the second and third sublists:
     *     It compares {@code list.get(k)} and {@code list.get(j)} using
     *     the {@code functional} instance and assigns the smaller element to {@code buffer.get(l++)}.
     *     It then increments the corresponding counters {@code j} and {@code k}.</li>
     *     <li>Elements remaining in the first and third sublists:
     *     It compares {@code list.get(k)} and {@code list.get(i)} using
     *     the {@code functional} instance and assigns the smaller element to {@code buffer.get(l++)}.
     *     It then increments the corresponding counters {@code i} and {@code k}.</li>
     *     <li>It handles the remaining elements in the first, second, and third sublists
     *     individually using separate {@code while} loops.
     *     It assigns the remaining elements to {@code buffer.get(l++)} and increments the corresponding counters.</li>
     * </ul>
     * {@code merge} method performs a three-way merge of sublists within the {@code list} by comparing elements using
     * the {@code functional} instance.
     * It maintains separate indices for each sublists and incrementally merges the elements into the {@code buffer}
     * list. After merging all the elements, it copies the merged elements back to the original {@code list}.
     * @param       list The list containing the sublists to be merged.
     * @param       left The starting index of the first sublists.
     * @param       mid1 The ending index (exclusive) of the first sublists.
     * @param       mid2 The ending index (exclusive) of the second sublists.
     ...
     * @see         SortFunctional#functionalCompareTo(Comparable, Comparable)
     */
    default <L extends T> void merge(
            List<L> list, int left, int mid1, int mid2, int right, List<L> buffer, SortFunctional<T> functional) {
        int i = left, j = mid1, k = mid2, l = left;
        while ((i < mid1) && (j < mid2) && (k < right)) {
    ...
    /**
     * {@code mergingInc} that performs a merging operation for the purpose of merging two sorted portions of a list
     * into a single sorted portion.
     * The method assumes that the left portion of the list, from index {@code left} to {@code mid}, and the
     * {@code right} portion,
     * from index {@code (mid + 1)} to {@code right}, are already sorted in increasing order.
     * <ul>
     *     <li>The {@link java.lang.SuppressWarnings @SuppressWarnings}{@code ("unchecked")} annotation is used to
     ...
     *     <li>The variables {@code n1} and {@code n2} are assigned the sizes of the left and right portions,
     *     respectively.</li>
     *     <li>The left sublist is obtained using {@link java.util.List#subList(int, int) subList} from {@code  left} to
     *     {@code (mid + 1)},
     ...
     *     <li>The subsequent {@code while} loop continues as long as both {@code i} and {@code j} are within their
     *     respective list sizes, {@code n1} and {@code n2}.
     *     It compares the elements at indices {@code i} and {@code j} of {@code leftArray} and {@code rightArray},
     *     respectively, and assigns the smaller element to {@code list.get(k)}.
     *     The corresponding index ({@code i} or {@code j}) and {@code k} are incremented accordingly.</li>
     *     <li>After the {@code while} loop, there might still be remaining elements in either {@code leftArray} or
     *     {@code rightArray}.
     *     The next two {@code while} loops handle the remaining elements by copying them into list starting from index
     *     {@code k}.</li>
     *     <li>Finally, the merging operation is complete, and the merged sorted portion is stored in the original
     *     {@code list} from index {@code left} to {@code right}.</li>
     * </ul>
     * {@code mergingInc} method is commonly used as part of merge sort algorithms to merge two sorted portions of a
     * list into a single sorted portion.
     * @param       list to be arranged.
     * @param       left from the list, index value, must be smaller than the {@code mid} and {@code right} value.
     * @param       mid from the list, index value, must be smaller than the {@code right} value, and must be greater
     *              than the {@code left}.
    ...
    /**
     * {@code mergingDec} that performs a merging operation for the purpose of merging two sorted portions of a list
     * into a single sorted portion.
     * The method assumes that the left portion of the list, from index {@code left} to {@code mid}, and the
     * {@code right} portion,
     * from index {@code (mid + 1)} to {@code right}, are already sorted in increasing order.
     ...
     *     {@code list} from index {@code left} to {@code right}.</li>
     * </ul>
     * {@code mergingDec} method is commonly used as part of merge sort algorithms to merge two sorted portions of a
     * list into a single sorted portion.
     * @param       list to be arranged.
     * @param       left from the list, index value, must be smaller than the {@code mid} and {@code right} value.
     * @param       mid from the list, index value, must be smaller than the {@code right} value, and must be greater
    ...
    /**
     * {@code merging} that performs a merging operation for the purpose of merging two sorted portions of a list into a
     * single sorted portion.
     * The method assumes that the left portion of the list, from index {@code left} to {@code mid}, and the
     * {@code right} portion,
     * from index {@code (mid + 1)} to {@code right},
     * and an instance of {@code mz.SortFunctional<T>} as parameters,
     ...
     *     <li>{@code merging} method now includes an additional check at the beginning.
     *     If the condition {@code (left > mid) || ((mid + 1) > right)} is satisfied,
     *     it means that either the left sublists is empty or the right sublists is empty,
     *     indicating that there is no need to perform the merging. In such cases, the method simply returns.</li>
     *     <li>It initializes variables {@code n1} and {@code n2} to store the sizes of the left and right sublists,
     *     respectively.
     *     The values are calculated as {@code (mid - left + 1)} and {@code (right - mid)}.</li>
     *     <li>It initializes counters {@code i}, {@code j}, and {@code k} to keep track of the current indices in the
     ...
     *     <li>It enters a {@code while} loop that continues as long as both {@code i} and {@code j} are within their
     *     respective
     *     sublists bounds ({@code (i < n1 && j < n2)}).
     *     Within the loop, it performs the comparison using
     *     the {@code functional} instance and assigns the smaller element to {@code list.get(k)}.
     *     After assigning the element, it increments the corresponding counters ({@code i}, {@code j}, and
     *     {@code k}).</li>
     *     <li>After exiting the {@code while} loop, it checks if there are any remaining elements in the left sublists
     *     {@code (i < n1)}.
     *     If so, it copies the remaining elements to {@code list.get(k)} and increments both {@code i} and
     *     {@code k}.</li>
     *     <li>Similarly, it checks if there are any remaining elements in the right sublists {@code (j < n2)}.
     *     If so, it copies the remaining elements to {@code list.get(k)} and increments both {@code j} and
     *     {@code k}.</li>
     * </ul>
     * {@code merging} method merges two sorted sublists ({@code leftArray} and {@code rightArray}) into a single sorted
     * list ({@code list}).
     * It compares elements using the {@code functional} instance and assigns the smaller element to {@code list.get(k)}
     * at each step.
     * The method handles cases where one of the sublists is empty or when there are remaining elements in either
     * the left or right sublists after the initial merging.
     * @param       list to be arranged.
     * @param       left from the list, index value, must be smaller than the {@code mid} and {@code right} value.
     * @param       mid from the list, index value, must be smaller than the {@code right} value, and must be greater
     *                  than the {@code left}.
    ...
    /**
     * {@code mergingInc} method for merging two sorted sublists within a list using the merge sort algorithm.
     * <ul>
     *     <li>{@link java.lang.SuppressWarnings @SuppressWarnings}{@code ("unchecked")}: This annotation suppresses
     *     unchecked warnings that might occur when using generics,
     *     indicating that the code will handle any necessary type checking appropriately.</li>
     *     <li>Initializes two pointers {@code n1} and {@code n2} to keep track of
     *     the current positions within the sublists to be merged.
     *     {@code n1} is initially set to the leftmost index, and {@code n2} is set to the index immediately after the
     *     midpoint.</li>
     *     <li>This loop iterates from the {@code left} index to the {@code right} index (inclusive) to merge the
     *     sublists.</li>
     *     <li>This condition checks if the current element at position {@code n1} in the left sublists is less than or
     *     equal to
     *     the current element at position {@code n2} in the right sublists.
     *     It also checks if the {@code n2} pointer has reached the end of the right sublists.
     *     If either of these conditions is true, the element at {@code n1} is selected for merging.</li>
     *     <li>If the condition in the previous step is true, the element at position {@code n1} in
     *     the left sublists is copied to the {@code buffer} list at position {@code i}.
     *     Then, the {@code n1} pointer is incremented.</li>
     *     <li>If the condition in step 5 is false, the element at position {@code n2} in the right sublists is selected
     *     for merging.</li>
     *     <li>The element at position {@code n2} in the right sublists is copied to the {@code buffer} list at position
     *     {@code i}.
     *     Then, the {@code n2} pointer is incremented.</li>
     ...
     *     We use subList again to get the sublist from {@code buffer} that corresponds to the range
     *     {@code (left, right + 1)}.</li>
     ...
     * After the merging is complete, the merged elements are copied back
     * {@link java.lang.System#arraycopy(Object, int, Object, int, int) System.arraycopy} to the original list.
     * @param       list to be arranged.
     * @param       left The index of the leftmost element of the sublists to be merged.
     * @param       mid The index of the midpoint element in the sublists to be merged.
     * @param       right The index of the rightmost element of the sublists to be merged.
    ...
    /**
     * {@code mergingDec} method for merging two sorted sublists within a list using the merge sort algorithm.
     * <ul>
     *     <li>{@link java.lang.SuppressWarnings @SuppressWarnings}{@code ("unchecked")}: This annotation suppresses
     *     unchecked warnings that might occur when using generics,
     *     indicating that the code will handle any necessary type checking appropriately.</li>
     ...
     *     <li>This loop iterates from the {@code left} index to the {@code right} index (inclusive) to merge the
     *     sublists.</li>
     *     <li>This condition checks if the current element at position {@code n1} in the left sublists is more than or
     *     equal to
     *     the current element at position {@code n2} in the right sublists.
     *     It also checks if the {@code n2} pointer has reached the end of the right sublists.
     *     If either of these conditions is true, the element at {@code n1} is selected for merging.</li>
     ...
     *     <li>If the condition in step 5 is false, the element at position {@code n2} in the right sublists is selected
     *     for merging.</li>
     *     <li>The element at position {@code n2} in the right sublists is copied to the {@code buffer} list at
     *     position {@code i}.
     *     Then, the {@code n2} pointer is incremented.</li>
     *     <li>First remove the elements from {@code list} using the {@link java.util.List#subList(int, int) subList}
     *     method and then clear the sublist using {@link java.util.List#clear() clear()}.</li>
     ...
     * and the indices {@code left}, {@code mid}, and {@code right} that define the range of elements to be merged.
     * <ul>
     *     <li>It initializes two variables: {@code n1} to track the index of the current element in the left sublists,
     *     and {@code n2} to track the index of the current element in the right sublists.</li>
     *     <li>It creates a {@code functionalAddEquals} instance using the {@code functionalComparableToAddEquals}
     *     method,
     *     which is responsible for defining the comparison logic for determining element order.</li>
     *     <li>It enters a {@code for} loop that iterates over the range from {@code left} to {@code right} (inclusive).
     *     Within the loop, it checks two conditions:</li>
     *     <li>If {@code n1} is within the range of the left sublists and either {@code n2} is out of the range of
     *     the right sublists or the comparison between the elements at indices {@code n2} and {@code n1} indicates
     *     that the element in the right sublists is greater or equal,
     *     it assigns the element from the left sublists to {@code buffer[i]} and increments {@code n1}.</li>
     *     <li>Otherwise, it assigns the element from the right sublists to {@code buffer[i]} and increments {@code n2}.</li>
     *     <li>First remove the elements from {@code list} using the {@link java.util.List#subList(int, int) subList}
     *     method and then clear the sublist using {@link java.util.List#clear() clear()}.</li>
     *     <li>Next, we use {@link java.util.List#addAll(int, Collection) addAll}
     *     to add the elements from {@code buffer} to {@code list}.
     ...
     *     ensuring that the elements from {@code buffer} are inserted at the correct position in {@code list}.</li>
     * </ul>
     * {@code merging} method merges two sorted sublists, specified by the indices {@code left}, {@code mid}, and
     * {@code right},
    ...
    /**
     * {@code mergingInc} method for merging two sorted sublists into a single list.
     * <ul>
     *     <li>{@link java.lang.SuppressWarnings @SuppressWarnings}{@code ("unchecked")}: This annotation suppresses
     *     unchecked warnings that might occur when using generics,
     *     indicating that the code will handle any necessary type checking appropriately.</li>
     ...
     *     <li>This {@code while} loop continues as long as both {@code i} and {@code j} are within the bounds of their
     *     respective sublists.</li>
     *     <li>{@code if (rightList.get(j).compareTo(leftList.get(i)) > 0)}:
     *     This condition checks if the element at position {@code i} in the left sublists is less than the element at
     *     position {@code j} in the right sublists.
     *     If true, the element from the left sublists is selected for merging.</li>
     *     <li>If the condition in the previous step is true, the element at position {@code i} in the left sublists is
     *     copied to the {@code list} at position {@code k}.
     *     Then, all three pointers {@code i, j, k} are incremented.</li>
     *     <li>If the condition in step 5 is false, the element at position {@code j} in the right sublists is selected
     *     for merging.</li>
     *     <li>The element at position {@code j} in the right sublists is copied to the {@code list} at position
     *     {@code k}.
     *     Then, all three pointers {@code i, j, k} are incremented.</li>
     *     <li>This {@code while} loop copies any remaining elements in the left sublists to the {@code list} if there
     *     are any.</li>
     *     <li>The remaining element at position {@code i} in the left sublists is copied to the {@code list} at
     *     position {@code k}.
     *     Then, both i and {@code k} pointers are incremented.</li>
     *     <li>This loop copies any remaining elements in the right sublists to the {@code list} if there are any.</li>
     *     <li>The remaining element at position {@code j} in the right sublists is copied to the {@code list} at
     *     position {@code k}.
     *     Then, both {@code j} and {@code k} pointers are incremented.</li>
     * </ul>
     * {@code mergingInc} this method merges two sorted sublists {@code leftList} and {@code rightList} into a single
     * sorted list {@code list}.
     * It compares elements from both sublists and copies them to the {@code list} in the correct order until one of the
     * sublists is fully processed.
     * Then, it copies any remaining elements from the unfinished sublists to the list.
     * @param       list to be arranged.
     * @param       leftList The left sublists that is sorted in ascending order.
     * @param       rightList The right sublists that is sorted in ascending order.
     */
    @SuppressWarnings("unchecked")
    ...
    /**
     * {@code mergingDec} method for merging two sorted sublists into a single list.
     * <ul>
     ...
     *     indicating that the code will handle any necessary type checking appropriately.</li>
     *     <li>Initializes three pointers {@code i, j, k}. {@code i} points to the current position in the left sublists,
     *     {@code j} points to the current position in the right sublists, and {@code k} points to the current position
     *     in the merged list.</li>
     *     <li>This {@code while} loop continues as long as both {@code i} and {@code j} are within the bounds of their
     *     respective sublists.</li>
     *     <li>{@code if (rightList.get(j).compareTo(leftList.get(i)) < 0)}:
     *     This condition checks if the element at position {@code i} in the left sublists is more than the element at
     *     position {@code j} in the right sublists.
     *     If true, the element from the left sublists is selected for merging.</li>
     *     <li>If the condition in the previous step is true, the element at position {@code i} in the left sublists is
     *     copied to the {@code list} at position {@code k}.
     ...
     *     Then, both {@code j} and {@code k} pointers are incremented.</li>
     * </ul>
     * {@code mergingDec} this method merges two sorted sublists {@code leftList} and {@code rightList} into a single
     * sorted list {@code list}.
     * It compares elements from both sublists and copies them to the {@code list} in the correct order until one of
     * the sublists is fully processed.
     * Then, it copies any remaining elements from the unfinished sublists to the list.
     * @param       list to be arranged.
     ...
     *     <li>Within the loop, it compares the current elements at indices i and j from leftList and rightList,
     *     respectively.
     *     If the comparison indicates that the element in {@code rightList} is considered greater than the element in
     *     {@code leftList},
     *     it assigns the element from {@code leftList} to {@code list.get(k)} and increments {@code i} and {@code k}.
     *     Otherwise, it assigns the element from {@code rightList} to {@code list.get(k)} and increments {@code j} and
     *     {@code k}.</li>
     *     <li>After the loop, it checks if there are any remaining elements in {@code leftList} and {@code rightList}
     *     that haven't been processed.
     ...
     * </ul>
     * {@code merging} method merges two sorted sublists, {@code leftList} and {@code rightList}, into a single sorted
     * list {@code list}.
     * It uses the comparison logic defined by the {@code SortFunctional} instance to determine the order of elements
     * during the merging process.
     * @param       list to be arranged.
     * @param       leftList The left sublists that is sorted in descending order.
     * @param       rightList The right sublists that is sorted in descending order.
