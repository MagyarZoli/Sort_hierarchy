...
public class Heap
     ...
     * It works by constructing a heap from the input array and repeatedly extracting
     * the maximum (or minimum) element from the heap and placing it at the end of the sorted portion of the
     * array.<br><br>
     * <b>Example:</b>
     * <ol>
     ...
     *     <li>{@code heapify} is a process where the element at index i is compared with its children, and if
     *     necessary,
     ...
     *     <li>Swap the root element (which is the maximum or minimum element depending on whether it's a max-heap or
     *     min-heap)
     ...
     *     <li>Repeat step 2 until all elements have been extracted from the heap. The extracted elements will be in
     *     the reverse order for
     *     a max-heap (sorted in ascending order) or in the correct order for a min-heap (sorted in descending
     *     order).</li>
     * </ol>
     * <b>Note:</b><br>
     * Heap Sort has a time complexity of <em>O(n log n)</em> in the average and worst cases, where <em>n</em> is the
     ...
     * Heap Sort is an in-place sorting algorithm, meaning it requires only a constant amount of additional memory
     * beyond the original array.
     * It is commonly used in situations where an in-place sorting algorithm is needed, or when the input data is too
     * large to fit into the main memory.<br><br>
     * <b>Property:</b><br>
     ...
     * Stability:               <b>No</b>
     * @see         IntroHeap#IntroHeap() IntroHeap
     * @see         SmoothHeap#SmoothHeap() SmoothHeap
     * @see         TernaryHeap#TernaryHeap() TernaryHeap
     * @see         WeakHeap#WeakHeap() WeakHeap
     */
    public Heap() {}
