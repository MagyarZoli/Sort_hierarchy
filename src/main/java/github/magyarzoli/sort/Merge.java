...
public class Merge
     ...
     *     <li>Compare the elements at the pointers and select the smaller (or larger) element to be placed in the
     *     merged list.</li>
     *     <li>Move the pointer of the selected element to the next position.</li>
     *     <li>Repeat the comparison and merging process until all elements from both sublists are merged into the
     *     final sorted list.</li>
     * </ol>
     * <b>Note:</b><br>
     * The key step in Merge Sort is the merging process, where two sorted sublists are combined to form a single
     * sorted list.
     * This process ensures that the elements are appropriately sorted during the merge phase.<br><br>
     * Merge Sort has a time complexity of <em>O(n log(n))</em> in the average and worst cases, where n is the number
     * of elements in the list.
     * It is considered an efficient sorting algorithm and is particularly useful for large datasets.
     * Merge Sort's time complexity remains the same regardless of the input sequence, making it a reliable choice for
     * sorting.<br><br>
     * Merge Sort is a stable sorting algorithm, meaning it maintains the relative order of elements with equal values.
     * It is an in-place algorithm in its purest form, meaning it can sort the elements using only a small, constant
     * amount of additional memory.
     * However, in most practical implementations, it requires auxiliary space proportional to the size of the input
     * list.<br><br>
     * Merge Sort is widely used in various applications due to its stability, efficiency, and ease of
