...
import github.magyarzoli.sort.intro.IntroSlow;
...
public class Slow
     ...
     * </ol>
     * <b>Note:</b><br>
     * Is a sorting algorithm. It is of humorous nature and not useful. It is a reluctant algorithm based on the
     * principle
     ...
     * Stability:               <b>No</b>
     * @see         IntroSlow#IntroSlow() IntroSlow
     */
    public Slow() {}
     ...
     * @param       array to be arranged.
     * @see         Slow#slowInc(Comparable[], int, int)
     */
     ...
     * is when the subarray has one element or is empty, which means it's already sorted.
     * @param       array to be arranged.
     * @see         Slow#slowDec(Comparable[], int, int)
     */
     ...
     * <ul>
     *     <li>It calls the overloaded version of the {@code slow} method with additional parameters: <i>0</i>
     *     represents
     ...
     *     which is a recursive divide-and-conquer algorithm.</li>
     * </ul>
     * By recursively calling the {@code slow} method with different subranges of the array and the custom comparison
     * logic,
     ...
     * @param       array to be arranged.
     * @param       functional lambda expression for comparison.
     * @see         Slow#slow(List, int, int, SortFunctional)
     */
     ...
     *     <li>Recursively call {@code slowInc} to sort the left half of the array from {@code left} to
     *     {@code mid}.</li>
     ...
     *     {@code (right - 1)}. This is done to ensure the rightmost element (the one previously at the
     *     {@code mid} index)
     ...
     * {@code left} and {@code right} indices indicating the range to be sorted. Additionally, it takes a
     * {@code functional}
     ...
     *     <li>Recursively call {@code slow} to sort the right half of the array from {@code (mid + 1)} to
     *     {@code right},
     *     using the provided {@code functional} for comparisons.</li>
     *     <li>After the left and right halves are sorted, the code uses the custom comparison logic provided
     *     through the
     ...
     *     position.</li>
     * </ul>
     * {@code slow} This implementation is still based on the divide-and-conquer approach of  but allows for
     * customizable
    ...
    /**
     * {@code slowInc} This method takes a {@link java.util.List List} of {@link java.lang.Comparable Comparable}
     * elements
     * ({@code L} should be a type that implements the {@code Comparable} interface) as input.
     * <ul>
     *     <li>It calls the overloaded version of {@code slowInc} with additional parameters: <i>0</i>
     *     represents the left
    ...
    /**
     * {@code slowDec} This method takes a {@link java.util.List List} of {@link java.lang.Comparable Comparable}
     * elements
     * ({@code L} should be a type that implements the {@code Comparable} interface) as input.
     * <ul>
     *     <li>It calls the overloaded version of {@code slowDec} with additional parameters: <i>0</i> represents
     *     the left
     *     index (starting point of the sorting), and {@code (list.size() - 1)} represents the right index
     *     (end point of the sorting). This effectively sorts the entire list.</li>
     *     <li>The sorting logic is defined in the {@code slowDec} method. The sorting process uses the algorithm,
     ...
     * {@code slowInc} This method takes a {@link java.util.List List} of {@link java.lang.Comparable Comparable}
     * elements
     * ({@code L} should be a type that implements the {@code Comparable} interface) as input, along with
     * {@code left} and
     * {@code right} indices indicating the range to be sorted.
     * <ul>
     *     <li>{@link java.lang.SuppressWarnings @SuppressWarnings}{@code ("unchecked")} This annotation is
     *     used to suppress
     *     compiler warnings. In this context, it is likely suppressing warnings related to the use of
     *     unchecked conversions
     *     (i.e., type safety) because of the generic type {@code L} extends {@code Comparable}.</li>
     *     <li>This is the base case of the recursive function. If the left index is greater than or
     *     equal to the right
     *     index, it means there is either one element or no elements to be sorted, so the function
     ...
     *     <li>Recursively call {@code slowInc} to sort the right half of the list from {@code (mid + 1)} to
     *     {@code right}.</li>
     *     <li>After the left and right halves are sorted, the code checks if the element at the {@code mid} index is
     *     greater than the element at the {@code right} index. If so, it means the right half contains a smaller value
     *     than the left half's maximum value. In order to ensure the elements are in ascending order, it swaps the
     ...
     *     <li>Finally, it makes a recursive call to sort the right half again, excluding the last element
     *     {@code (right - 1)}.
     *     This is done to ensure the rightmost element (the one previously at the {@code mid} index) is in
     *     its correct position.</li>
     * </ul>
     ...
     * {@code right} indices indicating the range to be sorted.
     * <ul>
     *     <li>{@link java.lang.SuppressWarnings @SuppressWarnings}{@code ("unchecked")} This annotation
     *     is used to suppress
     *     compiler warnings. In this context, it is likely suppressing warnings related to the use of
     *     unchecked conversions
     *     (i.e., type safety) because of the generic type {@code L} extends {@code Comparable}.</li>
     ...
     *     be used to split the list into two halves for sorting.</li>
     *     <li>Recursively call {@code slowInc} to sort the left half of the list from {@code left} to {@code mid}.</li>
     *     <li>Recursively call {@code slowInc} to sort the right half of the list from {@code (mid + 1)}
     *     to {@code right}.</li>
     *     <li>After the left and right halves are sorted, the code checks if the element at the {@code mid} index is
     *     greater than the element at the {@code right} index. If so, it means the right half contains a smaller value
     *     than the left half's maximum value. In order to ensure the elements are in descending order, it swaps the
     ...
     *     <li>Finally, it makes a recursive call to sort the right half again, excluding the last element
     *     {@code (right - 1)}.
     *     This is done to ensure the rightmost element (the one previously at the {@code mid} index) is
     *     in its correct position.</li>
     * </ul>
     ...
     * and {@code right} indices indicating the range to be sorted. Additionally, it takes a {@code functional}
     * parameter
     ...
     *     using the provided {@code functional} for comparisons.</li>
     *     <li>After the left and right halves are sorted, the code uses the custom comparison logic
     *     provided through the
     *     {@code functional} interface to compare the elements at the {@code mid} and {@code right}
     *     indices. If the custom
     *     comparison returns {@code true}, it means the element at {@code mid} should be placed after the element at
     *     {@code right} in the sorted order. Therefore, it swaps the elements at the {@code mid} and
     ...
     * @see         SortSwap#swap(List, int, int)
     */
    protected <L extends Comparable> void slow(
