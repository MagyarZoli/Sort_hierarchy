...
/**
 * IntroDPQ (Dual Pivot Quick) Sort is a hybrid sorting algorithm that combines the strengths of Dual Pivot Quick Sort
 * and Insertion Sort.
 ...
 * To mitigate the risk of Dual Pivot Quick Sort's worst-case behavior, Intro Sort monitors the
 ...
 * Additionally, for small subarrays, IntroSort switches to Insertion Sort, which has good
 * performance for small input sizes.
...
public interface IntroDPQ<T extends Comparable>
     ...
     *     it directly uses the {@code introSortClassInc} method to sort the sub-array
     *     using a different sorting algorithm.
     *     This is a base case to prevent excessive recursion.</li>
     *     <li>If the sub-array size is larger than {@code INTRO_SIZE} and the maximum
     *     recursion depth is not <i>0</i>, it performs the following steps:</li>
     ...
     *     If not, it uses insertion sort to sort the sub-array.</li>
     *     <li>If the maximum recursion depth {@code maxDepth} is <i>0</i>,
     *     it directly uses the {@code introSortClassDec} method to sort the
     *     sub-array using a different sorting algorithm.
     *     This is a base case to prevent excessive recursion.</li>
     *     <li>If the sub-array size is larger than {@code INTRO_SIZE} and the
     *     maximum recursion depth is not <i>0</i>, it performs the following steps:</li>
     ...
     * and an instance of the {@code SortFunctional<T>} interface as parameters.
     * <ul>
     ...
     *     <li>It calls the {@code partitionDual} method to select two pivot
     ...
     *     <li>At that point, it calls the {@code insertion} method to
     *     perform insertion sort on the remaining range.</li>
     * </ul>
