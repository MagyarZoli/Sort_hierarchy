...
/**
 * IntroDPQ (Dual Pivot Quick) Wiki Sort is a hybrid sorting algorithm that combines the strengths of
 * Dual Pivot Quick Sort and Wiki Sort.
 ...
 * To mitigate the risk of Dual Pivot Quick Sort's worst-case behavior, Wiki Sort monitors the
...
public class IntroDPQWiki
        extends IntroWiki
        implements IntroDPQ<Comparable> {

    /**
     * <b>IntroDPQ (Dual Pivot Quick) Wiki Sort:</b><br>
     * Is a hybrid sorting algorithm that combines the strengths of Dual Pivot Quick Sort and Wiki Sort.
     * It aims to provide fast average-case performance while maintaining worst-case guarantees.
     ...
     * If the depth exceeds a certain threshold, the algorithm switches to HeapSort,
     * which guarantees worst-case <em>O(n log(n))</em> time complexity but has higher overhead.
     * Additionally, for small subarrays, Intro switches to Wiki Sort, which has good performance
     ...
     *     it directly to sort the sub-array using a Intro Wiki sorting algorithm.
