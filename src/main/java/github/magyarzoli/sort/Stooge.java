...
/**
 * Stooge Sort is a recursive sorting algorithm that works by recursively sorting the first two-thirds
 * and last two-thirds
 * of the array until the array is sorted. It is similar to other divide-and-conquer algorithms, but
 * its division ratio is
...
public class Stooge
    ...
    /**
     * <b>Stooge Sort:</b><br>
     * Is a recursive sorting algorithm that works by recursively sorting the first two-thirds and last
     * two-thirds
     * of the array until the array is sorted. It is similar to other divide-and-conquer algorithms,
     * but its division ratio is
     ...
     *     <li>If the subarray size is 2 or smaller, the array is already sorted (base case), and no
     ...
     * other sorting algorithms like quicksort or mergesort, which have better average and worst-case
     * time complexities.<br><br>
     * Stability in sorting algorithms refers to whether equal elements maintain their relative order after sorting.
     * Unfortunately, Stooge sort is not a stable sorting algorithm. It means that the order of equal
     * elements may change
     ...
     * due to its inefficiency. However, understanding its recursive nature and the divide-and-conquer
    ...
    /**
     * {@code stoogeInc} method on the entire array. It calls the {@code stoogeInc} method with the
     ...
     * @param       array to be arranged.
     * @see         Stooge#stoogeInc(Comparable[], int, int)
     */
    ...
    /**
     * {@code stoogeDec} method on the entire array. It calls the {@code stoogeDec} method with the
     * initial left and right
     * indices set to <i>0</i> and {@code (array.length - 1)}, respectively, to sort the entire array.
     * <ul>
     *     <li>This method is the public entry point for the stooge sort algorithm. It takes a single argument, an array
     ...
     *     sort algorithm recursively divides the array into three parts and sorts them.</li>
     * </ul>
     * {@code stoogeDec} on an array, you can use the stooge sort algorithm to sort
     * the entire array in descending order.
     * @param       array to be arranged.
     * @see         Stooge#stoogeDec(Comparable[], int, int)
     */
    ...
    /**
     * {@code stooge} method on the entire array, with the added functionality of using a custom
     * comparison logic through
     * the {@code SortFunctional} functional interface.
     * <ul>
     *     <li>This method is the public entry point for the stooge sort algorithm with
     *     custom comparison logic. It takes
     *     two arguments, an array of {@link java.lang.Comparable Comparable} objects
     *     and a {@code functional} object that
     ...
     *     <li>The {@code stooge} method is responsible for performing the stooge sort
     *     algorithm on the entire array with
     *     the custom comparison logic provided by the {@code functional} object. The
     ...
     * @param       array to be arranged.
     * @param       functional lambda expression for comparison.
     * @see         Stooge#stooge(Comparable[], int, int, SortFunctional)
     */
     ...
     *     annotation is used to suppress compiler
     *     warnings related to type safety when using the {@link java.lang.Comparable#compareTo(Object)
     *     compareTo} method.</li>
     *     <li>The first condition {@code array[left].compareTo(array[right]) > 0} compares
     *     the elements at the {@code left}
     *     and {@code right} indices and swaps them if the element at the {@code left} index is greater than the element
     *     at the {@code right} index. This step ensures that the smallest element is at the
     *     beginning ({@code left}) and
     ...
     *     than or equal to <i>3</i> elements. If not, the method returns, as an array with <i>2</i>
     *     or fewer elements is
     *     already sorted.</li>
     *     <li>If the subarray size is greater than or equal to <i>3</i>, the {@code stoogeInc}
     ...
     *     at the {@code right} index. This step ensures that the greatest element is at the
     *     beginning ({@code left}) and
     *     the largest element is at the end ({@code right}) of the current subarray.</li>
     *     <li>The variable {@code third} is calculated as {@code ((right - left + 1) / 3)}, which represents one-third
     *     of the subarray's size.</li>
     *     <li>The next condition if {@code ((third * 3) >= 3)} checks if the size of the current subarray is greater
     *     than or equal to <i>3</i> elements. If not, the method returns, as an array with
     *     <i>2</i> or fewer elements is
     *     already sorted.</li>
     *     <li>If the subarray size is greater than or equal to <i>3</i>, the {@code stoogeDec}
     *     method is called recursively
     *     three times:</li>
     *     <li>The first recursive call sorts the first two-thirds of the subarray:
     ...
     * the array. Instead of calling {@code compareTo}, it will call the {@code functionalCompareTo}
     * method on the provided
     * functional object to perform the comparison.
     * <ul>
     *     <li>The first condition {@code functional.functionalCompareTo(array[left], array[right])}
     *     compares the elements
     *     at the {@code left} and {@code right} indices using the custom comparison logic
     *     provided by the {@code functional}
     ...
     *     <li>If the subarray size is greater than or equal to <i>3</i>, the {@code stooge}
     *     method is called recursively
     *     three times:</li>
     *     <li>The first recursive call sorts the first two-thirds of the subarray:
     ...
     * <ul>
     *     <li>This method entry point for the stooge sort algorithm on a {@link java.util.List List}.
     *     It takes a single
     *     argument, a {@code List} of elements {@code List<L> list} that extend the
     *     {@link java.lang.Comparable Comparable}
     *     interface.</li>
     *     <li>{@code stoogeInc(list, 0, (list.size() - 1))} This line of code calls the
     *     recursive {@code stoogeInc} method,
     ...
     *     by the {@code left} and {@code right} indices. As described in the previous
     *     responses, the stooge sort algorithm
     *     recursively divides the list into three parts and sorts them.</li>
     * </ul>
     * By calling {@code stoogeInc} on a list, you can use the stooge sort algorithm to
     * sort the entire list in ascending
     ...
     * @see         Stooge#stoogeInc(List, int, int)
     */
     ...
     *     {@link java.lang.Comparable Comparable}
     *     interface.</li>
     *     <li>{@code stoogeDec(list, 0, (list.size() - 1))} This line of code calls the recursive
     *     {@code stoogeDec} method,
     *     passing the entire list as the input and setting the initial left and right indices to <i>0</i> and
     *     {@code (list.size() - 1)}, respectively.</li>
     *     <li>{@code stoogeDec} method is responsible for performing the stooge sort
     *     algorithm on the sublist specified
     *     by the {@code left} and {@code right} indices. As described in the previous
     *     responses, the stooge sort algorithm
     *     recursively divides the list into three parts and sorts them.</li>
     * </ul>
     * By calling {@code stoogeDec} on a list, you can use the stooge sort algorithm t
     * o sort the entire list in descending
     * order based on the natural ordering of the elements (as they extend the {@code Comparable} interface). The
     * {@code stoogeInc} method assumes the natural ordering of elements for comparison.
     * @param       list to be arranged.
     * @param       <L> allows this method to be used with different types.
     * @see         Stooge#stoogeDec(List, int, int)
     */
     ...
     * {@code functional} object. This makes the Stooge sort algorithm adaptable to a wide
     * range of data types and custom
    ...
    /**
     * {@code stoogeInc} method that takes three parameters list of elements {@code List<L> list},
     * the {@code left} index,
     ...
     * <ul>
     *     <li>{@link java.lang.SuppressWarnings @SuppressWarnings}{@code ("unchecked")} This
     *     annotation is used to suppress
     *     compiler warnings. In this context, it is likely suppressing warnings related to
     *     the use of unchecked conversions
     *     (i.e., type safety) because of the generic type {@code L} extends {@code Comparable}.</li>
     *     <li>If {@code (list.get(left).compareTo(list.get(right)) > 0)} This line compares
     *     the elements at the {@code left}
     *     and {@code right} indices in the list using their natural ordering defined by the
     *     {@code Comparable} interface.
     *     If the element at the {@code left} index is greater than the element at the
     *     {@code right} index, it swaps them
     *     using the {@code swap} method.</li>
     *     <li>The variable {@code third = ((right - left + 1) / 3)} calculates the size of
     *     one-third of the sublist.</li>
     *     <li>This condition checks if the sublist size is greater than or equal to <i>3</i>
     *     elements. If not, the method
     *     returns, as a sublist with <i>2</i> or fewer elements is already sorted.</li>
     *     <li>If the sublist size is greater than or equal to <i>3</i>, the {@code stoogeInc}
     *     method is called recursively
    ...
    /**
     * {@code stoogeDec} method that takes three parameters list of elements {@code List<L> list},
     * the {@code left} index,
     * and the {@code right} index of the portion of the list to be sorted. The generic type {@code L} extends the
     * {@link java.lang.Comparable Comparable} interface, ensuring that elements in the list can be compared.
     * <ul>
     ...
     *     <li>If {@code (list.get(left).compareTo(list.get(right)) < 0)} This line compares
     *     the elements at the {@code left}
     *     and {@code right} indices in the list using their natural ordering defined by
     *     the {@code Comparable} interface.
     *     If the element at the {@code left} index is smaller than the element at the
     *     {@code right} index, it swaps them
     *     using the {@code swap} method.</li>
     *     <li>The variable {@code third = ((right - left + 1) / 3)} calculates the size
     *     of one-third of the sublist.</li>
     *     <li>This condition checks if the sublist size is greater than or equal to
     *     <i>3</i> elements. If not, the method
     *     returns, as a sublist with <i>2</i> or fewer elements is already sorted.</li>
     *     <li>If the sublist size is greater than or equal to <i>3</i>, the
     *     {@code stoogeDec} method is called recursively
     *     three times to sort the sublist:</li>
     *     <li>The first recursive call sorts the first two-thirds of the sublist:
     ...
     * the {@code right} index of the portion of the list to be sorted, and a {@code functional}
     * object implementing the
     * {@code SortFunctional<Comparable>} interface.
     * <ul>
     *     <li>If {@code (functional.functionalCompareTo(list.get(left), list.get(right)))} This
     *     line compares the elements
     ...
     *     <li>The variable {@code third = ((right - left + 1) / 3)} calculates the size of
     *     one-third of the sublist.</li>
     *     <li>If {@code ((third * 3) >= 3)} This condition checks if the sublist size is
     *     greater than or equal to <i>3</i>
     ...
     * @see         SortSwap#swap(List, int, int)
     */
    protected <L extends Comparable> void stooge(
