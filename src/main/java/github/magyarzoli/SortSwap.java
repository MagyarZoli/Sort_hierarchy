...
extends Sorter<T> {
     ...
     * two elements in order to maintain the decreasing order of the array.
     * @param       array to be arranged.
     * @param       i elements to be compared
     * @param       j elements to be compared
     * @return      the value of swapped.
     ...
     *     This annotation is not directly related to the functionality of the method but rather a way to handle warnings.</li>
     *     <li>The variable {@code swap} is initially set to {@code false}, indicating that no swap operation has been performed yet.</li>
     *     <li>The condition {@code if (list.get(i).compareTo(list.get(i + j)) > 0)} compares the elements at indices {@code i} and {@code (i + j)} in
     *     the array to check if a swap is necessary. If the element at index {@code i} is greater than the element
     *     at index {@code (i + j)}, a swap operation is performed.</li>
     *     <li>Inside the {@code if} block, the swap method is called to {@code swap} the elements at indices {@code i} and {@code (i + j)} in the list.
     *     After the {@code swap}, the swapped variable is set to {@code true}.</li>
     *     <li>Finally, the method returns the value of swapped, indicating whether a swap operation
     *     was performed ({@code true}) or not ({@code false}).</li>
     * </ul>
     * {@code isSwapInc} method can be used as a helper method in sorting algorithms to check if a swap is necessary between
     * two elements in order to maintain the increasing order of the list.
     ...
     * </ul>
     * {@code isSwapInc} method can be used as a helper method in sorting algorithms to check if a swap is necessary between
     * two elements in order to maintain the decreasing order of the list.
     * @param       list to be arranged.
     * @param       i elements to be compared
     * @param       j elements to be compared
     * @return      the value of swapped.
    ...
    /**
     * {@code isSwap} that takes a list {@code list}, two indices {@code i} and {@code j}, and a {@code CompareTo<T>} function as parameters.
     * The method attempts to swap the elements at indices i and j in the list based on
     * the result of the comparison performed by the {@code functionalCompareTo} method.
     * <ul>
     *     <li>Calls the {@code functionalCompareTo} method from the {@code CompareTo<T>} function object,
     *     passing the elements at indices {@code i} and {@code (i + j)} of the array as arguments.
